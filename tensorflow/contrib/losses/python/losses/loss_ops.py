...
def huber(y_true, y_pred, delta):
    residual = math_ops.abs(y_true - y_pred)

    def f1():
        return 0.5 * math_ops.square(residual)

    def f2():
        return delta * residual - 0.5 * math_ops.square(delta)

    return control_flow_ops.cond(residual <= delta, f1, f2)
